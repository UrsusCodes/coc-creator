...
- **Hosting:** GitHub Pages at `https://ursuscodes.github.io/coc-creator/`. Deploy is automated via `.github/workflows/deploy.yml` — triggered ONLY on push to `master`. Vite `base: '/coc-creator/'`. **There is no Vercel / Netlify** — older docs that say so are inaccurate.
- **Backend:** Supabase.
